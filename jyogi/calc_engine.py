...
def calculate_current_dasha(moon_long_deg: float, birth_date_str: str) -> str:
    ...
    # FIX: use UTC so calculation is identical on Mac, Streamlit Cloud, Render, Railway
    # datetime.now() uses server local time which differs by 5h30m on cloud servers
    current_date = datetime.now(pytz.utc).replace(tzinfo=None)
