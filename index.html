...
<title>Jyogi AI — Vedic Astrology · Tarot · Sacred Crystals</title>
<meta name="description" content="India's most accurate AI-powered Vedic astrology, tarot readings, and sacred crystal bracelets. Book a session with Jyogi today.">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;600;700&family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;1,300;1,400;1,500&family=Cormorant+SC:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>
...
:root{
  ...
  --text:#f5ecd7;--muted:#9d8c6a;--dim:#6b5e44;
  --wa:#25D366;
}
html{scroll-behavior:smooth;}
body{background:var(--bg);color:var(--text);font-family:'Cormorant Garamond',serif;font-size:17px;line-height:1.7;overflow-x:hidden;}
...
.nav-logo-sub{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;color:var(--muted);display:block;margin-top:-2px;}
.nav-links{display:flex;gap:6px;align-items:center;}
.nav-link{font-family:'Cormorant SC',serif;font-size:11px;letter-spacing:0.25em;color:var(--muted);padding:6px 14px;border-radius:20px;transition:all 0.2s;cursor:pointer;}
.nav-link:hover,.nav-link.active{color:var(--gold);background:rgba(255,195,64,0.08);}
.nav-wa{background:linear-gradient(135deg,var(--green),var(--green2));color:#fff!important;border-radius:20px;padding:8px 18px;font-size:11px;letter-spacing:0.2em;display:flex;align-items:center;gap:7px;transition:all 0.25s;}
...
.mobile-nav{display:none;position:fixed;inset:0;z-index:99;background:rgba(3,1,10,0.97);backdrop-filter:blur(20px);flex-direction:column;align-items:center;justify-content:center;gap:24px;}
.mobile-nav.open{display:flex;}
.mobile-nav .nav-link{font-size:14px;padding:10px 24px;}

/* ── HERO ── */
.hero{position:relative;z-index:1;min-height:100svh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:100px 24px 60px;overflow:hidden;}
.hero-mandala{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:min(700px,90vw);height:min(700px,90vw);opacity:0.04;animation:spin 120s linear infinite;}
@keyframes spin{to{transform:translate(-50%,-50%) rotate(360deg);}}
.hero-eye{font-family:'Cormorant SC',serif;font-size:10px;letter-spacing:0.5em;color:var(--gold);opacity:0.7;margin-bottom:18px;animation:fadeUp 1.2s ease both;}
.hero-title{font-family:'Cinzel',serif;font-size:clamp(32px,7vw,72px);font-weight:700;color:#fff;margin-bottom:8px;animation:fadeUp 1.2s 0.1s ease both;line-height:1.1;}
...
.hero-sub{font-family:'Cormorant Garamond',serif;font-size:clamp(15px,2.5vw,21px);color:rgba(245,236,215,0.7);margin-bottom:36px;max-width:680px;font-style:italic;animation:fadeUp 1.2s 0.2s ease both;}
.hero-pills{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-bottom:44px;animation:fadeUp 1.2s 0.3s ease both;}
.hero-pill{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;padding:6px 16px;border:1px solid rgba(255,195,64,0.3);border-radius:20px;color:var(--gold-lt);}
.hero-ctas{display:flex;gap:14px;flex-wrap:wrap;justify-content:center;animation:fadeUp 1.2s 0.4s ease both;}
.btn{display:inline-flex;align-items:center;gap:9px;padding:14px 30px;border-radius:30px;font-family:'Cormorant SC',serif;font-size:12px;letter-spacing:0.3em;cursor:pointer;border:none;transition:all 0.3s;text-transform:uppercase;}
...
.hero-scroll{position:absolute;bottom:32px;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:8px;animation:fadeUp 1.5s 0.8s ease both;}
.hero-scroll-txt{font-family:'Cormorant SC',serif;font-size:8px;letter-spacing:0.5em;color:var(--dim);}
...
.sec-badge{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.45em;color:var(--gold);margin-bottom:14px;}
.sec-title{font-family:'Cinzel',serif;font-size:clamp(22px,4vw,36px);color:var(--text);margin-bottom:16px;}
.sec-title span{color:var(--gold);}
.sec-desc{font-size:16px;color:var(--muted);font-style:italic;max-width:600px;margin:0 auto;}
...
/* ── STAT BAR ── */
.stats{background:linear-gradient(135deg,var(--card),rgba(43,122,11,0.08));border:1px solid var(--border);border-radius:16px;display:grid;grid-template-columns:repeat(4,1fr);padding:32px;gap:24px;margin-bottom:72px;}
.stat{text-align:center;}
.stat-num{font-family:'Cinzel',serif;font-size:28px;color:var(--gold);font-weight:700;}
.stat-lbl{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;color:var(--muted);margin-top:4px;}
...
.svc-name{font-family:'Cinzel',serif;font-size:15px;color:var(--text);margin-bottom:4px;}
.svc-price{font-family:'Cormorant SC',serif;font-size:13px;color:var(--gold);margin-bottom:10px;}
.svc-duration{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.2em;color:var(--dim);margin-bottom:12px;}
.svc-desc{font-size:14px;color:var(--muted);line-height:1.6;margin-bottom:18px;}
.svc-includes{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:18px;}
.svc-tag{font-size:11px;padding:3px 10px;background:rgba(255,195,64,0.07);border:1px solid rgba(255,195,64,0.15);border-radius:10px;color:var(--muted);}
.btn-book{width:100%;padding:11px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--wa),#1fa854);color:#fff;font-family:'Cormorant SC',serif;font-size:11px;letter-spacing:0.25em;cursor:pointer;transition:all 0.25s;display:flex;align-items:center;justify-content:center;gap:8px;}
...
.spread-tab{padding:9px 20px;border-radius:20px;border:1px solid var(--border);background:transparent;color:var(--muted);font-family:'Cormorant SC',serif;font-size:10px;letter-spacing:0.25em;cursor:pointer;transition:all 0.25s;}
...
.card-slot-label{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;color:var(--dim);margin-bottom:12px;}
...
.card-name{font-family:'Cinzel',serif;font-size:11px;color:var(--text);display:block;}
.card-orient{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.15em;margin-top:2px;}
...
.cmp-orient-badge{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;padding:3px 12px;border-radius:10px;display:inline-block;margin-bottom:14px;}
...
.form-label{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.3em;color:var(--muted);}
.form-input{background:rgba(0,0,0,0.35);border:1px solid var(--border);border-radius:10px;padding:12px 16px;color:var(--text);font-size:15px;font-family:'Cormorant Garamond',serif;outline:none;transition:border-color 0.25s;width:100%;}
...
.bracelet-badge{position:absolute;top:14px;left:14px;font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.15em;padding:4px 12px;border-radius:20px;color:#fff;}
.bracelet-price-bubble{position:absolute;bottom:14px;right:14px;background:rgba(3,1,10,0.85);backdrop-filter:blur(10px);border:1px solid var(--bhi);border-radius:10px;padding:6px 12px;text-align:center;}
.bpb-price{font-family:'Cinzel',serif;font-size:15px;color:var(--gold);font-weight:700;}
.bpb-original{font-size:12px;color:var(--dim);text-decoration:line-through;}
.bracelet-body{padding:20px 22px;}
.bracelet-name{font-family:'Cinzel',serif;font-size:16px;color:var(--text);margin-bottom:3px;}
.bracelet-sub{font-size:14px;color:var(--gold);margin-bottom:6px;}
.bracelet-tagline{font-style:italic;font-size:14px;color:var(--muted);margin-bottom:14px;}
.bracelet-planet{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.2em;color:var(--dim);margin-bottom:8px;}
.bracelet-benefits{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:14px;}
.benefit-tag{font-size:11px;padding:3px 9px;border-radius:8px;background:rgba(255,195,64,0.05);border:1px solid rgba(255,195,64,0.12);color:var(--muted);}
.bracelet-ritual{font-size:13px;color:var(--muted);font-style:italic;margin-bottom:14px;padding:10px 12px;background:rgba(43,122,11,0.05);border-left:2px solid rgba(43,122,11,0.3);border-radius:0 8px 8px 0;}
.btn-order{width:100%;padding:13px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--wa),#1fa854);color:#fff;font-family:'Cormorant SC',serif;font-size:11px;letter-spacing:0.25em;cursor:pointer;transition:all 0.25s;display:flex;align-items:center;justify-content:center;gap:8px;}
...
.review-text{font-size:15px;color:var(--text);font-style:italic;line-height:1.7;margin-bottom:16px;}
.review-product{font-family:'Cormorant SC',serif;font-size:9px;letter-spacing:0.2em;color:var(--green2);margin-bottom:12px;}
.review-user{display:flex;align-items:center;gap:10px;}
.review-avatar{width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,var(--gold2),var(--green));display:flex;align-items:center;justify-content:center;font-family:'Cinzel',serif;font-size:14px;font-weight:700;color:#fff;flex-shrink:0;}
.review-name{font-family:'Cinzel',serif;font-size:13px;color:var(--text);}
.review-location{font-size:12px;color:var(--dim);}
...
.footer-wa-btn{display:inline-flex;align-items:center;gap:8px;padding:12px 22px;background:linear-gradient(135deg,var(--wa),#1fa854);color:#fff;border-radius:24px;font-family:'Cormorant SC',serif;font-size:10px;letter-spacing:0.25em;transition:all 0.25s;}
.footer-wa-btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(37,211,102,0.3);}
.footer-col h4{font-family:'Cinzel',serif;font-size:11px;color:var(--text);margin-bottom:16px;letter-spacing:0.05em;}
...
/* ── WA FLOAT ── */
.wa-float{position:fixed;bottom:28px;right:28px;z-index:200;width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,var(--wa),#1fa854);display:flex;align-items:center;justify-content:center;font-size:26px;box-shadow:0 8px 28px rgba(37,211,102,0.35);cursor:pointer;transition:all 0.3s;animation:pulse-wa 3s infinite;}
...
@media(max-width:900px){
  ...
  .god-grid{grid-template-columns:1fr;max-width:320px;margin:0 auto;}
...
@media(max-width:600px){
  ...
  .god-grid{max-width:100%;}
  .god-img-ring{width:140px;height:140px;}
}

/* ── TOOLTIP ── */
[data-tip]{position:relative;cursor:help;}
[data-tip]:hover::after{content:attr(data-tip);position:absolute;bottom:calc(100% + 6px);left:50%;transform:translateX(-50%);background:rgba(15,4,40,0.95);border:1px solid var(--border);border-radius:6px;padding:6px 10px;font-size:12px;white-space:nowrap;font-style:normal;color:var(--text);z-index:10;}
...
<nav id="main-nav">
  <div class="nav-inner">
    <a class="nav-logo" href="#hero">
      <div class="nav-logo-glyph">🔮</div>
      <div>
        <span class="nav-logo-text">Jyogi AI</span>
        <span class="nav-logo-sub">✦ Vedic · Tarot · Sacred ✦</span>
      </div>
    </a>
    <div class="nav-links">
      <span class="nav-link" onclick="scrollTo('#services')">Services</span>
      <span class="nav-link" onclick="scrollTo('#tarot')">Tarot</span>
      <span class="nav-link" onclick="scrollTo('#astrology')">Astrology</span>
      <span class="nav-link" onclick="scrollTo('#shop')">Shop</span>
      <span class="nav-link" onclick="scrollTo('#reviews')">Reviews</span>
      <a class="nav-link nav-wa" href="https://wa.me/918310319870?text=Hi Jyogi! I would like to book a session." target="_blank">
        <span>💬</span> Book Now
      </a>
    </div>
...
<div class="mobile-nav" id="mobile-nav">
  <span class="nav-link" onclick="scrollTo('#services');toggleMobileNav()">Services</span>
  <span class="nav-link" onclick="scrollTo('#tarot');toggleMobileNav()">Tarot</span>
  <span class="nav-link" onclick="scrollTo('#astrology');toggleMobileNav()">Astrology</span>
  <span class="nav-link" onclick="scrollTo('#shop');toggleMobileNav()">Shop</span>
  <span class="nav-link" onclick="scrollTo('#reviews');toggleMobileNav()">Reviews</span>
  <a class="btn btn-wa" href="https://wa.me/918310319870?text=Hi Jyogi! I would like to book a session." target="_blank" onclick="toggleMobileNav()">💬 Book Now on WhatsApp</a>
</div>

<!-- ── HERO ── -->
<section class="hero" id="hero">
  <svg class="hero-mandala" viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="200" cy="200" r="180" stroke="#FFC340" stroke-width="0.5"/>
    <circle cx="200" cy="200" r="150" stroke="#FFC340" stroke-width="0.5"/>
    <circle cx="200" cy="200" r="120" stroke="#FFC340" stroke-width="0.5"/>
    <circle cx="200" cy="200" r="90" stroke="#FFC340" stroke-width="0.5"/>
    <line x1="20" y1="200" x2="380" y2="200" stroke="#FFC340" stroke-width="0.5"/>
    <line x1="200" y1="20" x2="200" y2="380" stroke="#FFC340" stroke-width="0.5"/>
    <line x1="72.7" y1="72.7" x2="327.3" y2="327.3" stroke="#FFC340" stroke-width="0.5"/>
    <line x1="327.3" y1="72.7" x2="72.7" y2="327.3" stroke="#FFC340" stroke-width="0.5"/>
    <polygon points="200,40 348,290 52,290" stroke="#FFC340" stroke-width="0.5" fill="none"/>
    <polygon points="200,360 52,110 348,110" stroke="#FFC340" stroke-width="0.5" fill="none"/>
    <circle cx="200" cy="200" r="12" stroke="#FFC340" stroke-width="0.5"/>
  </svg>
  <div class="hero-eye">✦ Jyogi AI — Vedic Astrology · Tarot · Sacred Crystals ✦</div>
  <h1 class="hero-title">Your <span>Stars</span> Hold<br>Every Answer</h1>
  <p class="hero-sub">India's most precise AI-powered Vedic readings, Light Seer Tarot, and sacred crystal bracelets — charged with ancient wisdom.</p>
  <div class="hero-pills">
    <span class="hero-pill">🪐 Sidereal Lahiri Astrology</span>
    <span class="hero-pill">🎴 Light Seer Tarot</span>
    <span class="hero-pill">💎 Crystal Bracelets</span>
    <span class="hero-pill">🔢 Vedic Numerology</span>
  </div>
  <div class="hero-ctas">
    <span class="btn btn-primary" onclick="scrollTo('#tarot')">🎴 Draw Your Cards</span>
    <span class="btn btn-outline" onclick="scrollTo('#astrology')">🪐 Get My Chart</span>
    <a class="btn btn-wa" href="https://wa.me/918310319870?text=Hi Jyogi! I would like to book a session." target="_blank">💬 Book on WhatsApp</a>
  </div>
  <div class="hero-scroll">
    <span class="hero-scroll-txt">Scroll to explore</span>
    <div class="scroll-chevron"></div>
  </div>
</section>

<!-- ── STATS ── -->
<section style="padding:0 24px 72px;" id="stats">
  <div class="container">
    <div class="stats">
      <div class="stat"><div class="stat-num">5,000+</div><div class="stat-lbl">Readings Delivered</div></div>
      <div class="stat"><div class="stat-num">4.9★</div><div class="stat-lbl">Average Rating</div></div>
      <div class="stat"><div class="stat-num">12+</div><div class="stat-lbl">Years Experience</div></div>
      <div class="stat"><div class="stat-num">78</div><div class="stat-lbl">Card Light Seer Deck</div></div>
    </div>
  </div>
</section>
...
<section id="services">
  <div class="container">
    <div class="sec-header">
      ...
      <h2 class="sec-title">Sacred <span>Services</span></h2>
      <p class="sec-desc">Each reading is powered by ancient Vedic wisdom and AI precision — deeply personal, always accurate.</p>
    </div>
...
<section id="tarot">
  <div class="container-md">
    <div class="sec-header">
      <div class="sec-badge">✦ LIGHT SEER'S TAROT ✦</div>
      <h2 class="sec-title">Draw Your <span>Cards</span></h2>
      <p class="sec-desc">The Light Seer deck by Chris-Anne speaks the language of your soul. Ask your question and let the cards answer.</p>
    </div>
    <div class="divider" style="margin-bottom:48px;"></div>
    <div class="tarot-inner">
      <div class="spread-tabs" id="spread-tabs">
        <button class="spread-tab active" data-spread="single" onclick="selectSpread('single')">Single Card</button>
        <button class="spread-tab" data-spread="three" onclick="selectSpread('three')">Past · Present · Future</button>
        <button class="spread-tab" data-spread="celtic" onclick="selectSpread('celtic')">Celtic Cross</button>
        <button class="spread-tab" data-spread="love" onclick="selectSpread('love')">Love & Relationship</button>
        <button class="spread-tab" data-spread="career" onclick="selectSpread('career')">Career & Money</button>
      </div>
      <textarea class="tarot-question" id="tarot-question" rows="3" placeholder="Focus your mind and type your question... or leave empty for a general reading."></textarea>
      <button class="draw-btn" id="draw-btn" onclick="drawCards()">✨ Draw Your Cards</button>
      <div class="cards-display" id="cards-display"></div>
      <div class="card-meaning-panel" id="card-meaning-panel"></div>
    </div>
  </div>
</section>
...
<section id="astrology">
  <div class="container">
    <div class="sec-header">
      <div class="sec-badge">✦ VEDIC ASTROLOGY ✦</div>
      <h2 class="sec-title">Your <span>Birth Chart</span></h2>
      <p class="sec-desc">Enter your birth details for a complete Sidereal Vedic chart with Lagna, Nakshatras, Dasha periods, and Sadhe Sati analysis.</p>
    </div>
    <div class="divider" style="margin-bottom:48px;"></div>
    <div class="astro-form-wrap">
      <div class="form-grid">
        <div class="form-group">
          <label class="form-label">Full Name</label>
          <input class="form-input" id="astro-name" type="text" placeholder="Your name">
        </div>
        <div class="form-group">
          <label class="form-label">Date of Birth</label>
          <input class="form-input" id="astro-dob" type="date">
        </div>
        <div class="form-group">
          <label class="form-label">Time of Birth</label>
          <input class="form-input" id="astro-time" type="time">
        </div>
        <div class="form-group">
          <label class="form-label">City of Birth</label>
          <input class="form-input" id="astro-city" type="text" placeholder="Mumbai, Delhi, Bengaluru…">
        </div>
        <div class="form-group full">
          <label class="form-label">Your Question (optional)</label>
          <input class="form-input" id="astro-question" type="text" placeholder="What would you like guidance on? Career, relationships, health…">
        </div>
      </div>
      <button class="submit-btn" onclick="submitAstrology()">🪐 Calculate My Vedic Chart</button>
      <div id="astro-result" class="astro-result"></div>
      <div class="api-note">
        🔗 <strong>Powered by Swiss Ephemeris</strong> — Sidereal Lahiri calculation, Vimshottari Dasha engine, and Sadhe Sati analysis. Results delivered via the Jyogi AI backend.
      </div>
    </div>
  </div>
</section>
...
<section id="shop">
  <div class="container">
    <div class="sec-header">
      <div class="sec-badge">✦ SACRED CRYSTALS ✦</div>
      <h2 class="sec-title">Crystal <span>Bracelets</span></h2>
      <p class="sec-desc">Each bracelet is hand-selected, cleansed under the full moon, and charged with planetary mantras. Order via WhatsApp for personalised guidance.</p>
    </div>
    <div class="divider" style="margin-bottom:48px;"></div>
...
<section id="reviews">
  <div class="container">
    <div class="sec-header">
      ...
      <h2 class="sec-title">What <span>Seekers</span> Say</h2>
      <p class="sec-desc">Real transformations from real people across India.</p>
    </div>
    <div class="divider" style="margin-bottom:48px;"></div>
    <div class="reviews-scroll" id="reviews-grid">
      <!-- rendered by JS -->
    </div>
  </div>
</section>
...
<section id="gallery">
  <div class="container">
    <div class="sec-header">
      <div class="sec-badge">✦ SACRED GALLERY ✦</div>
      <h2 class="sec-title">Divine <span>Blessings</span></h2>
      <p class="sec-desc">We begin every reading with a prayer to the divine. May these sacred energies guide your path.</p>
    </div>
    <div class="divider" style="margin-bottom:48px;"></div>
    <div class="god-grid" id="god-grid">
      <!-- rendered by JS -->
    </div>
  </div>
</section>
  ...
  <div class="footer-inner">
    <div class="footer-grid">
      <div class="footer-brand">
        <span class="nav-logo-text">Jyogi AI</span>
        <span class="nav-logo-sub" style="letter-spacing:0.35em;font-size:9px;color:var(--muted);font-family:'Cormorant SC',serif;margin-bottom:12px;display:block;">✦ Vedic Astrology · Tarot · Sacred Crystals ✦</span>
        <p>Ancient wisdom meets modern clarity. Every soul has a map written in the stars — let Jyogi help you read it.</p>
        <a class="footer-wa-btn" href="https://wa.me/918310319870?text=Hi Jyogi! I would like to book a session." target="_blank">💬 Chat on WhatsApp</a>
      </div>
      <div class="footer-col">
        <h4>Services</h4>
        <div class="footer-links">
          <span class="footer-link" onclick="scrollTo('#services')">Vedic Reading</span>
          <span class="footer-link" onclick="scrollTo('#services')">Live Tarot</span>
          <span class="footer-link" onclick="scrollTo('#services')">Crystal Prescription</span>
          <span class="footer-link" onclick="scrollTo('#services')">Numerology</span>
        </div>
      </div>
      <div class="footer-col">
        <h4>Tools</h4>
        <div class="footer-links">
          <span class="footer-link" onclick="scrollTo('#tarot')">Tarot Reader</span>
          <span class="footer-link" onclick="scrollTo('#astrology')">Birth Chart</span>
          <span class="footer-link" onclick="scrollTo('#shop')">Crystal Shop</span>
        </div>
      </div>
      <div class="footer-col">
        <h4>Connect</h4>
        <div class="footer-links">
          <a href="https://wa.me/918310319870" target="_blank">WhatsApp</a>
    ...
    <div class="footer-bottom">
      <span>© 2025 Jyogi AI · All readings are for spiritual guidance and entertainment</span>
      <span>Made with 🙏 &amp; ✨ in India</span>
    </div>
  </div>
</footer>

<a class="wa-float" href="https://wa.me/918310319870?text=Hi Jyogi! I would like to book a session." target="_blank" title="Chat on WhatsApp">💬</a>
...
(function(){
  ...
  function init(){
    resize();stars=[];
    for(let i=0;i<220;i++){
      stars.push({x:Math.random()*c.width,y:Math.random()*c.height,r:Math.random()*1.4+0.2,a:Math.random(),da:(Math.random()-0.5)*0.004});
    }
  }
  function draw(){
...
const SERVICES = [
  ...
  {id:'tarot_session',name:'Live Tarot Session',icon:'🎴',price:'₹800',duration:'30 min',
    desc:'One-on-one tarot reading via WhatsApp video. Ask your burning question and receive guidance from the Light Seer deck.',
    includes:['3-card or 5-card spread','Light Seer Tarot deck','Voice explanation','WhatsApp video call','Recorded session'],
...
const BRACELETS = [
  {id:'pyrite_citrine',name:'Money Magnet',sub:'Pyrite & Citrine',tagline:'Attract wealth. Amplify abundance.',
    price:'₹1,499',original:'₹2,000',planet:'Jupiter / Sun',chakra:'Solar Plexus',chakraColor:'#FFD700',
...
const REVIEWS = [
  {user:'Anjali S.',location:'Pune',rating:5,product:'Money Magnet Bracelet',avatar:'A',text:'Got a promotion I had been waiting two years for — within weeks of wearing this! The energy shift was real and immediate.'},
  {user:'Rahul K.',location:'Delhi',rating:5,product:'Vedic Reading',avatar:'R',text:'Jyogi identified my Saturn transit challenge and gave remedies that actually worked. Her predictions were frighteningly accurate.'},
  {user:'Priya M.',location:'Mumbai',rating:5,product:'Self-Love Mala',avatar:'P',text:'Beautiful quality crystals. The Rose Quartz mala feels genuinely peaceful — I wear it every single day. My relationship with myself transformed.'},
  {user:'Deepak R.',location:'Bangalore',rating:5,product:'Live Tarot Session',avatar:'D',text:'The live tarot reading gave me complete clarity about my career change. Every card spoke directly to my situation. Mind-blowing.'},
  {user:'Meena T.',location:'Hyderabad',rating:5,product:'Protection Shield',avatar:'M',text:'The negative energy in my office genuinely shifted after placing the tourmaline on my desk. My colleagues even noticed the change in atmosphere.'},
  {user:'Sanjay P.',location:'Kolkata',rating:5,product:'Full Vedic Reading',avatar:'S',text:'The PDF report is incredibly detailed. I have referred to it every month for a year and the predictions keep coming true one by one.'},
  {user:'Kavita L.',location:'Jaipur',rating:5,product:'Third Eye Awakener',avatar:'K',text:'Started having vivid dreams and strong gut feelings after wearing this for a week. My meditation practice deepened immediately.'},
  {user:'Arjun N.',location:'Chennai',rating:5,product:'Crystal Prescription',avatar:'A',text:'The personalised crystal prescription was worth every rupee. Jyogi explained exactly which stones suit my chart — no generic advice.'},
  {user:'Sunita V.',location:'Ahmedabad',rating:5,product:'Lucky Charm',avatar:'S',text:'Wore the Lucky Charm to my business pitch and landed a ₹40L contract. Coincidence? I do not think so. Jyogi is the real deal.'},
];
...
function renderShop(){
  ...
  g.innerHTML=BRACELETS.map(b=>`
    <div class="bracelet-card">
      <div class="bracelet-img-wrap">
        <img src="${b.img}" alt="${b.name}" loading="lazy" onerror="this.src='/products/${b.id}.jpg'">
...
function drawCards(){
  ...
  setTimeout(()=>{
        ...
        <div class="card-frame">
          <div class="card-img-wrap">
            <img 
              ...
              loading="lazy"
              onerror="this.outerHTML='<div class=\"card-back-design\">🔮</div>'"
    ...
    if(drawn.length===1) showCardMeaning(0);
...
function showCardMeaning(idx){
  const card=window._drawnCards?.[idx];
  if(!card) return;
  const panel=document.getElementById('card-meaning-panel');
  panel.innerHTML=`
    <div class="cmp-card-name">${getCardName(card.name)}</div>
    <div>
      <span class="cmp-orient-badge ${card.reversed?'reversed':'upright'}">${card.reversed?'🔄 Shadow Seer (Reversed)':'✦ Light Seer (Upright)'}</span>
    </div>
...
// ─── ASTROLOGY ────────────────────────────────────────────────────────────
async function submitAstrology(){
  ...
  const question=document.getElementById('astro-question').value.trim();
  const result=document.getElementById('astro-result');
  
  if(!name||!dob||!city){
    result.innerHTML='<p style="color:#f87171;font-style:italic;">Please fill in your name, date of birth, and city.</p>';
    result.className='astro-result visible';
    return;
  }
  
  result.innerHTML='<div class="loading-ring" style="margin-top:8px;"></div><p style="text-align:center;margin-top:12px;font-style:italic;color:var(--muted);">Calculating your Vedic chart…</p>';
  result.className='astro-result visible';
  
  try {
    const resp=await fetch('https://jyogi-api.onrender.com/api/chart',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({name,dob,time,city,question})
    });
    if(!resp.ok) throw new Error('API error');
    const data=await resp.json();
    result.innerHTML=`
      <h3 style="font-family:'Cinzel',serif;color:var(--gold);margin-bottom:16px;">✦ ${data.name || name}'s Vedic Chart</h3>
      <div style="white-space:pre-wrap;font-size:15px;line-height:1.8;color:var(--text);">${data.report || data.text || JSON.stringify(data,null,2)}</div>
    `;
  } catch(e) {
    result.innerHTML=`
      <h3 style="font-family:'Cinzel',serif;color:var(--gold);margin-bottom:12px;">✦ Chart Ready — Book Your Full Reading</h3>
      <p style="color:var(--muted);font-style:italic;margin-bottom:16px;">Your details have been noted. To receive your complete Vedic chart with Lagna, Dasha analysis, Sadhe Sati, and planetary remedies — book a live session with Jyogi via WhatsApp.</p>
      <a href="https://wa.me/${WA_NUMBER}?text=Hi Jyogi! I would like a Vedic chart reading. Name: ${name}, DOB: ${dob}, City: ${city}${question?', Question: '+question:''}" target="_blank" style="display:inline-flex;align-items:center;gap:8px;padding:12px 24px;background:linear-gradient(135deg,#25D366,#1fa854);color:#fff;border-radius:24px;font-family:'Cormorant SC',serif;font-size:11px;letter-spacing:0.25em;text-decoration:none;">💬 Send My Chart Details to Jyogi</a>
    `;
  }
}
